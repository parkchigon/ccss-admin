...
function  preView(){
	...
	CKEDITOR.instances.pnotiContEdit.setData(notiCont);
...
function noticeUpdate() {
	...
	if(confirm("해당 공지사항 정보를 수정 하시겠습니까?")) {
		...
		var url = "<c:url value='/notice/updateNotice.do'/>";
		...
		if (formValidationCheck(checkParamArr)){
			$.ajax({
				...
				if(result == 'Y'){ 
					alert(" 공지 사항 정보가 수정 되었습니다.");
					location.href="<c:url value='/notice/noticeList.do' />";
				}else{
					alert("공지 사항 정보 수정에 실패하였습니다.");
				}
			}).error(function(request,status,error){
				//alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
				if(request.status == 401){
...
<form id="noticeUpdateForm" name="noticeUpdateForm">
			...
			<table class="table simple table_write_type">
				...
				<tbody>
					...
					<tr id="pushStartDtArea">
						<th><div class="tit_search">푸시 알림 시작 일시<span class="imp">*</span></div></th>
						<td colspan="3" class="last">
							...
							<select class="select w01" style="width: 100px;" id="spostMinute" name="spostMinute">
								<c:forEach var="minutes" begin="00" end="59">
									...
									<option value="${minute}" <c:if test="${empty noticeVO.notiId and minute eq 00}">selected="selected"</c:if> 
																<c:if test="${!empty noticeVO.notiId and minute eq noticeVO.spostMinute}">selected="selected"</c:if>>${minute}</option>
								</c:forEach>
							</select>
							&nbsp;분 &nbsp;
						</td>
					...
					<tr id="pushEndDtArea">
						<th><div class="tit_search">푸시 알림 종료 일시<span class="imp">*</span></div></th>
						<td colspan="3" class="last">
							...
							<select class="select w01" style="width: 100px;" id="epostHour" name="epostHour">
								<c:forEach var="hours" begin="00" end="23">
									<fmt:formatNumber var="hour" value="${hours}" pattern="00" />
									<option value="${hour}" <c:if test="${empty noticeVO.notiId and hour eq 12}">selected="selected"</c:if>
															 <c:if test="${!empty noticeVO.notiId and hour eq noticeVO.epostHour}">selected="selected"</c:if> >${hour}</option>
								</c:forEach>
							</select>
							 &nbsp;시 &nbsp;
							<select class="select w01" style="width: 100px;" id="epostMinute" name="epostMinute">
								<c:forEach var="minutes" begin="00" end="59">
									<fmt:formatNumber var="minute" value="${minutes}" pattern="00" />
									<option value="${minute}" <c:if test="${empty noticeVO.notiId and minute eq 00}">selected="selected"</c:if> 
																<c:if test="${!empty noticeVO.notiId and minute eq noticeVO.epostMinute}">selected="selected"</c:if>>${minute}</option>
								</c:forEach>
							</select>
							&nbsp;분 &nbsp;
						</td>
					...
					<tr>
						...
						<td colspan="3" class="last">
							...
							<textarea id="notiContEdit" name="notiContEdit" class="input textareabox" style="height:400px" readOnly><c:out value="${noticeVO.notiCont}" /></textarea>
							<script>
		...
		<div class="btn_wrap02">
			...
			<a href="javascript:noticeUpdate();">
									...
									<table class="table simple table_write_type">
										...
										<tbody>
											...
											<tr>
												...
												<!-- <textarea id="notiCont" name="notiCont" class="input textareabox" style="height:400px"></textarea> -->
													<!-- <textarea name="notiContEdit" id="notiContEdit" class="input textareabox" style="height:400px"></textarea> -->
													<textarea id="pnotiContEdit" name="pnotiContEdit" class="input textareabox" style="height:400px" readOnly></textarea>
													<script>
														// Replace the <textarea id="editor1"> with a CKEditor
														// instance, using default configuration.
														CKEDITOR.editorConfig = function( config ) {
														...
														CKEDITOR.replace( 'pnotiContEdit',
														{
															enterMode:'2',
