...
$(document).ready(function(){
	...
	//var serviceCategory = "${cardVO.serviceCategory}";
	//$("#serviceCategory").val(serviceCategory);
	
	
	///////////////////////////////////////////////////////////////////////////////////
	// 최초 등록된 첫번째(1)와 열두번째(12) 카드일 경우에는 카드명과 App ID외에는 수정이 될 수 없다
	///////////////////////////////////////////////////////////////////////////////////
	$("input[name=exposureYn]").click(function(e) {
		if (cardSortNum == 1 || cardSortNum == 12){
			alert("최초 적용 고정카드는 상태값을 변경할 수 없습니다.");
			e.preventDefault();
		}
	});
	$("input[name=fixYn]").click(function(e) {
		if (cardSortNum == 1 || cardSortNum == 12){
			alert("최초 적용 고정카드는 카드고정여부를 변경할 수 없습니다.");
			e.preventDefault();
		}
	});
	$("input[name=cardSortNum]").click(function(e) {
		if (cardSortNum == 1 || cardSortNum == 12){
			alert("최초 적용 고정카드는 카드번호를 변경할 수 없습니다.");
			e.preventDefault();
		}
	});
...
function cardUpdate() {
	...
	if(confirm("해당 서비스 카드 정보를 수정 하시겠습니까?")) {
		...
		var url = "<c:url value='/card/updateCard.do'/>";
		...
		if (formValidationCheck(checkParamArr)){
			$.ajax({
				url : url
				,data : $('#cardUpdateForm').serialize()
				,dataType : 'json'
				,type : "POST"
			}).done(function(data) {
				var result = data.result;
				if(result == 'Y'){ 
					alert("신규 서비스 카드가 정보가 수정 되었습니다.");
					location.href="<c:url value='/card/cardList.do' />";
				}else if(result =='DCN'){
					alert("서비스 카드 수정 실패  : 중복 되는 도메인 명이 있습니다.");
				}else if(result =='DCSN'){
					alert("서비스 카드 수정 실패  : 중복 되는 고정 카드 번호가 가 있습니다.");
				}else{
...
<form id="cardUpdateForm" name="cardUpdateForm">
		<input type="hidden" id="cardId" name="cardId" value="${cardVO.cardId}" />
		
		<div class="main_top">
			<h2> 카드덱관리 > 서비스 카드 관리 > 서비스 카드 수정</h2>
		</div>
				...
				<tbody>
					<tr>
						...
						<td colspan="3" class="last">
							<input type="text"  autocomplete="off" id="cardNm" name="cardNm"  value ="${cardVO.cardNm}" class="input w01" />
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td id=""colspan="3" class="last">
							<input type="text"  autocomplete="off" id="cardAppId" name="cardAppId" value ="${cardVO.cardAppId}" class="input w01" />
						</td>
					</tr>
					<tr>
					...
					<tr id="cardNumArea" style="display:none;">
