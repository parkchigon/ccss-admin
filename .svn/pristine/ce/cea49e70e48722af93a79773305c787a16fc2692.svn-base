...
function goSearch(page) {
	$("#page").val(page);
	$.ajax({
		url : "/admin/stats/selectLifeNumberStatListViewAjax.do",
		data : $("#lifeNumberFrom").serialize(),
		dataType : "JSON",
		type : "POST"
	}).done(function(data) {
		console.log(data);
		...
		if(resultCode == "1001") {
			...
			if(resultList.length > 0) {
				for(var i=0; i<resultList.length; i++) {
					...
					html += '	<td>' +resultList[i].businessName+ '</td>';
					html += '	<td>' +resultList[i].categoryName+ '</td>';
					html += '	<td>' +resultList[i].insertDate+ '</td>';
					html += '	<td>' +addComma(resultList[i].lguSerachListCnt)+ '</td>';
					if(resultList[i].categoryName == "콜택시"){
						html += '<td>-</td>';
					} else{
						html += '<td>' +addComma(resultList[i].lguSerachDetailCnt)+ '</td>';
					}
					html += '	<td>' +addComma(resultList[i].lguCallListCnt)+ '</td>';
					if(resultList[i].categoryName == "콜택시"){
						html += '<td>-</td>';
					}else{
						html += '<td>' +addComma(resultList[i].lguCallDetailCnt)+ '</td>';
					}
					html += '</tr>';
				}
			} else {
			...
			$("#lifeNumberList").empty();
			$("#lifeNumberList").append(html);
...
function downloadExcel() {
	$("#lifeNumberFrom").attr("action", "/admin/stats/selectLifeNumberStatListExcel.do");
	$("#lifeNumberFrom").submit();
}
</script>
<!-- 내용-->
<div class="contents_wrap">
	<form id="lifeNumberFrom" name="lifeNumberFrom" method="POST" >
				...
				<tr>
					...
					<td colspan="3">
						<select id="orderType" name="orderType" class="select w02">
							...
							<option value="SERACHCNT">조회순</option>
							<option value="BUSINESSNAME">업종명순</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<div class="input_wrap">
							<select id="keywordType" name="keywordType" class="select w02">
								<option value="BUSINESS_NAME" selected="selected">업종</option>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="table simple table_list_type">
			...
			<thead>
				<tr>
					...
					<th rowspan="2">상호 목록 진입 건수</th>
					<th rowspan="2">상호 상세 진입 건수</th>
					<th colspan="2">통화 호출 건수</th>
				</tr>
				<tr>
					<th>리스트</th>
					<th>상세</th>
