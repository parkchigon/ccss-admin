...
function downLoadExcel(){
	$('#smsStatSearchForm').attr("action", "<c:url value='/stat/selectSmsStatListExcel.do'/>");
	$('#smsStatSearchForm').submit();
}
...
function goSearch(page) {
	...
	$("#page").val(page);
	$.ajax({
		url : "<c:url value='/stat/selectSmsStatList.do'/>",
		data : $('#smsStatSearchForm').serialize(),
		dataType : 'json',
		type : "POST"
	}).done(function(data) {
...
<div class="contents_wrap">
	<form id="smsStatSearchForm" name="smsStatSearchForm">
	<input type="hidden" id="page" name="page" />
		<div class="main_top">
			<h2> 서비스 통계 > SMS 전송 이력 조회 </h2>
		</div>
		...
		<div class="table_wrap">
			...
			<div class="thead_wrap cboth">
				<div class="ltr">
					<div class="tit_table">SMS 전송 이력 리스트</div>
				</div>
				<div class="rtl">
