...
function selectGrpCodeSet(){
	selectGrpCodeList("CAR_PUSH_MSG_STATUS","msgStatus");
...
function resetSearch(){
	...
	$("#carPushStatList").empty();
	$("#carPushStatList").append(html);
	$("#totCnt").text("0");
...
function downLoadExcel(){
	$('#carPushStatSearchForm').attr("action", "<c:url value='/stat/selectCarPushStatListExcel.do'/>");
	$('#carPushStatSearchForm').submit();
}
...
function goSearch(page) {
	...
	$("#page").val(page);
	$.ajax({
		url : "<c:url value='/stat/selectCarPushStatList.do'/>",
		data : $('#carPushStatSearchForm').serialize(),
		dataType : 'json',
		type : "POST"
	}).done(function(data) {
...
<div class="contents_wrap">
	<form id="carPushStatSearchForm" name="carPushStatSearchForm">
	<input type="hidden" id="page" name="page" />
		<div class="main_top">
			<h2> 서비스 통계 > Car Push 전송 이력 조회 </h2>
		</div>
		...
		<div class="table_wrap">
			...
			<div class="thead_wrap cboth">
				<div class="ltr">
					<div class="tit_table">Car Push 전송 이력 리스트</div>
				</div>
				<div class="rtl">
